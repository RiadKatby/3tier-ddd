...
$(window).load(function () {
    ...
    $('input[type="submit"].click-once , button[type="submit"].click-once').click(function () {
        ...
        if ($(this).attr('name') && $(this).attr('value')) {
            if ($("input[type='hidden'][name='" + $(this).attr('name') + "']").length) {
...
$(document).on('change', 'select[data-ajax="true"]', function (event) {
    ...
    var duration = $(this).data('ajax-loading-duration') || 0;
...
function renderModelStateErrors(formSelector, modelStateErrors) {
    ...
    for (var i = 0; i < modelStateErrors.length; i++) {
        ...
        if (!element) //try to get encrypted version        
            element = $('#' + encrypterPrefix + modelStateErrors[i].key)[0];
        if (!element) continue;
...
$(document).on("change", ".checkbox_appended input:checkbox", function () {
    ...
    var range = textBox.attr('data-year-range') || '1300:c+20';
    ...
    try {
        ...
        if (textBox.val()) {
            var jdDate = calendarFrom.parseDate("dd/mm/yyyy", textBox.val()).toJD();
        ...
        var newYearRange = yearRange[0] + ':' + yearRange[1];

        textBox.calendarsPicker('option', 'yearRange', newYearRange);
