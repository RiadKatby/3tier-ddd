/*
  ...
  /*
  $('#OfficeNameTemp').filter_input({ regex: '[\u0600-\u06FF \\s]*'});
*/
    ...
    $.fn.extend({
        ...
        filter_input: function (options) {
            ...
            function filter_input_function(event) {
                ...
                var callback = input.attr('filter-input-invalid-callback');
                if (callback) {
                    try {
                        var fn = new Function("input", callback + '(input);');
                        fn(input[0]);
                    } catch (e) { }
                }
                return false;
            ...
            if (options.live) {
                if (parseInt(jquery_version[0]) >= 1 && parseInt(jquery_version[1]) >= 7) {
                    if (options.selector) {
                        $(document).on(options.events, options.selector, filter_input_function);
                    }
                    else {
                    ...
                    if (options.selector) {
                        $(document).live(options.events, options.selector, filter_input_function);
                    }
                    else {
