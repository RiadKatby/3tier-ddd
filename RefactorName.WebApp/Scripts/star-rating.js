    ...
    Rating.prototype = {
        ...
        listen: function () {
            ...
            self.listenClick(self.$rating, function (e) {
                ...
                self.setStars(pos);
                self.$element.trigger('change').trigger('rating.change', [self.$element.val(), self.$caption.html(), self.lastValue]);
        ...
        clear: function () {
            ...
            self.setStars();
            self.$element.trigger('rating.clear', [self.lastValue]);
        },
        reset: function () {
            var self = this;
            self.lastValue = self.$element.val();
            ...
            self.setStars();
            self.$element.trigger('rating.reset', [self.lastValue]);
        },
        update: function (val) {
            var self = this;
            if (!arguments.length) {
        ...
        //added by fadi omar
        showLoading: function (loadingElementDuration) {
            if (typeof loadingElementDuration === "undefined") loadingElementDuration = 500;
            var self = this;
            self.timer && clearTimeout(self.timer);
            self.timer = setTimeout(function () {
                if (!self.$container.find('i.fa-spin').length)
                    $('<i class="fa fa-refresh fa-spin"></i>').appendTo(self.$container);
            }, loadingElementDuration);
        },
        hideLoading: function () {
            var self = this;
            self.timer && clearTimeout(self.timer);
            self.$container.find('i.fa-spin').remove();
        }
